...
A full list of error codes is below — they're stable across releases, safe to switch on.`,
    errors: [
      ...
      { code: "unknown_provider", http: 400, when: "Connecting a DNS integration with a provider key we don't support." },
      { code: "missing_credentials", http: 400, when: "POST /dns-integrations body has no `credentials` object." },
      { code: "missing_display_name", http: 400, when: "POST /dns-integrations body has no `display_name` string." },
      { code: "invalid_credentials", http: 400, when: "Provider client failed to construct from the supplied credentials (e.g. malformed service-account JSON)." },
      { code: "credentials_rejected", http: 400, when: "Credentials parsed but the upstream API refused them — likely a typo or missing scopes." },
      { code: "zone_list_failed", http: 502, when: "Provider API rejected the zones-list call. We surface its error verbatim." },
      { code: "name_taken", http: 409, when: "A DNS integration with that `display_name` already exists for this customer." },
      { code: "integration_in_use", http: 409, when: "Trying to delete a DNS integration that still owns CNAMEs on active domains. Remove those domains first." },
  ...
  },
  {
    ...
    endpoints: [
      ...
      { id: "domains-remove", method: "DELETE", path: "/apps/{id}/domains/{hostname}", title: "Remove a domain", description: "Unmaps the hostname. Revokes the cert. If the domain was added via auto-config, we also delete the upstream CNAME at your DNS provider.", scope: "domains:write", stub: true,
  ...
  {
    id: "dns-integrations",
    title: "DNS providers (auto-config)",
    intro: `Connect Cloudflare, AWS Route 53, Google Cloud DNS, Vercel, or DigitalOcean so we can create the CNAME for a custom hostname at your DNS provider automatically. Credentials are encrypted at rest with AES-256-GCM and decrypted only when we call the upstream API.

When you POST a hostname under \`/apps/{id}/domains\` and an integration's zones cover the hostname's apex, the response will include an \`auto_configured\` object with the provider name, zone, and the upstream record ID. Otherwise you'll get the legacy "add a CNAME yourself" payload — no behaviour change.`,
    endpoints: [
      { id: "dns-providers-meta", method: "GET", path: "/dns-providers", title: "List supported providers + credential fields", description: "Static metadata. Use this to render a connect form: each provider declares its `fields` (name/label/type/hint) and whether it supports apex CNAMEs.", scope: "domains:read", since: "2026-06-04",
        examples: { curl: curlGet("/dns-providers"), js: jsGet("/dns-providers"), response: JSON.stringify([{ key: "cloudflare", label: "Cloudflare", docs_url: "https://dash.cloudflare.com/profile/api-tokens", fields: [{ name: "api_token", label: "API token", type: "password" }], apex_supported: true }], null, 2) } },
      { id: "dns-list", method: "GET", path: "/dns-integrations", title: "List connected providers", description: "Every DNS integration on this customer, with cached zone list + last test result.", scope: "domains:read", since: "2026-06-04",
        examples: { curl: curlGet("/dns-integrations"), js: jsGet("/dns-integrations") } },
      { id: "dns-create", method: "POST", path: "/dns-integrations", title: "Connect a provider", description: "Validates the credentials against the upstream API, fetches the zone list, and stores the integration. Returns 400 if the credentials don't work. Body: `{ provider, display_name, credentials }`.", scope: "domains:admin", since: "2026-06-04", tryBody: { provider: "cloudflare", display_name: "Cloudflare — prod", credentials: { api_token: "cf_pat_…" } },
        examples: { curl: curlBody("POST", "/dns-integrations", { provider: "cloudflare", display_name: "Cloudflare — prod", credentials: { api_token: "cf_pat_…" } }), js: jsBody("POST", "/dns-integrations", { provider: "cloudflare", display_name: "Cloudflare — prod", credentials: { api_token: "cf_pat_…" } }) } },
      { id: "dns-get", method: "GET", path: "/dns-integrations/{id}", title: "Get one provider", description: "Detail view, including the cached zone list and last test status.", scope: "domains:read", since: "2026-06-04",
        examples: { curl: curlGet("/dns-integrations/{id}"), js: jsGet("/dns-integrations/{id}") } },
      { id: "dns-test", method: "POST", path: "/dns-integrations/{id}/test", title: "Test connection", description: "Re-validates the stored credentials with a cheap upstream call. Updates `last_test_at` / `last_test_ok` / `last_test_error`.", scope: "domains:read", since: "2026-06-04",
        examples: { curl: curlBody("POST", "/dns-integrations/{id}/test", {}), js: jsBody("POST", "/dns-integrations/{id}/test", {}) } },
      { id: "dns-zones", method: "POST", path: "/dns-integrations/{id}/zones", title: "Refresh zone list", description: "Re-fetches zones from the upstream provider and replaces the cache. Use this if you've just created a new zone at your provider and want to use it for auto-config.", scope: "domains:read", since: "2026-06-04",
        examples: { curl: curlBody("POST", "/dns-integrations/{id}/zones", {}), js: jsBody("POST", "/dns-integrations/{id}/zones", {}) } },
      { id: "dns-delete", method: "DELETE", path: "/dns-integrations/{id}", title: "Disconnect", description: "Removes the integration. Returns 409 if there are active domains still owned by this integration — remove those domains first.", scope: "domains:admin", since: "2026-06-04",
        examples: { curl: `curl -X DELETE ${BASE}/${SLUG}/dns-integrations/{id} -H "Authorization: Bearer ${TOKEN}"`, js: `await fetch("${BASE}/${SLUG}/dns-integrations/{id}", { method: "DELETE", headers: { Authorization: "Bearer ${TOKEN}" } });` } },
    ],
  },
  {
    id: "cron",
